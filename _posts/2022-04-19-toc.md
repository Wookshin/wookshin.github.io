...
Vanilla JS를 사용해서 ToC(Table of Contents) 구현할 텐데요.  
...
h1, h2 태그를 가져오기 위해 `document.querySelectorAll` 함수를 사용할 거에요. 
...
페이지 이동은 h1, h2 태그가 가지고 있는 `scrollIntoView` 함수를 사용할 거에요.
