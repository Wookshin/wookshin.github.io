...
exec 메서드는 인수로 전달받은 문자열에 대해 정규 표현식의 패턴을 검새갛여 매칭 결과를 배열로 반환합니다.  
...
대소문자를 구별하지 않고 검색하려면 플래그 i를 사용합니다.
...
따라서 [0-9] 와 같은 의미의 \d 와 반대로 동작하는 \D 는 [^0-9] 와 같고, [A-Za-z0-9_] 와 같은 의미의 \w 와 반대로 동작하는 \W 는 [^a-za-z0-9_] 와 같습니다.
