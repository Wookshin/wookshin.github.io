...
제목에는 15분만에 구현이라 썼지만, 개념 설명이 추가되어 글이 길게 느껴지네요!  
...
마크다운을 쓰면 #이 h1 태그로, ##이 h2 태그로 변환됩니다. (###은 h3로, ####은 h4로)  
제 글의 경우 h1 태그를 제목으로 h2 태그를 목차 제목으로 사용하기에 h1, h2 태그 내용을 ToC에 넣고자 합니다.  
...
태그 내용은 textContent 속성으로 가져올 수 있습니다.  
...
페이지 이동은 h1, h2태그가 가지고 있는 `scrollIntoView` 함수를 사용할거에요.
...
ToC 안에 있는 toc-item 클래스명을 가진 li태그를 클릭했을 때(ex. 1. 어원, 2. 코끼리의 몸 등) 매칭되는 태그로 페이지를 이동시켜야 합니다.  
click 했을 때 어떤 li 태그인지 파악하기 위해 index를 엘리먼트 속성에 저장해놓겠습니다.
그리고 li태그에 매칭되는 태그는 tags 변수에 리스트로 잘 담아두었으니 이를 활용하여 페이지를 이동시키면 됩니다.  

Toc를 만들었던 위 페이지에서 하기 js를 콘솔 창에 입력해보세요.
